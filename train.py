...
def main():
    ...
    test_sub_lit = dataset.split_valid_data(test_data, valid_num)
    dev_test_iter_lit = []
    for ite in range(1, valid_num + 1):
        _, dev_data, test_data = dataset.separate_train_dev_test(test_sub_lit, ite)
    ...
    """MODEL"""
    if model_type == 'multi':
        model = model_define.Multi(src_vocab_size, trg_vocab_size, embed_size, hidden_size, class_size, dropout_ratio, coefficient, src_initialW, trg_initialW)
    elif model_type in ['label', 'pretrain']:
        model = model_define.Label(src_vocab_size, trg_vocab_size, embed_size, hidden_size, class_size, dropout_ratio, src_initialW, trg_initialW)
    else:
        model = model_define.EncoderDecoder(src_vocab_size, trg_vocab_size, embed_size, hidden_size, dropout_ratio, src_initialW, trg_initialW)
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        for ite, lit in enumerate(dev_test_iter_lit, start=1):
            ...
            logger.info('E{} ##  {}: {}\tdev: {}, micro: {}, macro: {} {}'.format(epoch, ite, param, dev_score, test_micro_score, dataset.float_to_str(rate), test_macro_score))
        ...
        logger.info('E{} ## loss: {}, dev: {}, micro: {}, macro: {} {}'.format(epoch, train_loss, ave_dev_score, ave_micro_score, dataset.float_to_str(ave_test_score), ave_macro_score))
        logger.info('')

        label, align, tf = dataset.sort_multi_list(id_total, label_total, align_total, tf_total)
        dataset.save_list(model_save_dir + 'model_epoch_{}.label'.format(epoch), label)
        dataset.save_list(model_save_dir + 'model_epoch_{}.align'.format(epoch), align)
        dataset.save_list(model_save_dir + 'model_epoch_{}.tf'.format(epoch), tf)
