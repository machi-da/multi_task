...
def main():
    ...
    qa_train_data = dataset.load_label_corpus_file(train_src_file, train_trg_file)
    qa_valid_data = dataset.load_label_corpus_file(valid_src_file, valid_trg_file)
    ...
    """VOCABULARY"""
    src_vocab, trg_vocab, sos, eos = dataset.prepare_vocab(base_dir, qa_train_data, vocab_size, gpu_id)
    ...
    if not load_model:
        ...
        train_iter = dataset.Iterator(qa_train_data, src_vocab, trg_vocab, batch_size, gpu_id, sort=True, shuffle=True)
        valid_iter = dataset.Iterator(qa_valid_data, src_vocab, trg_vocab, batch_size, gpu_id, sort=False, shuffle=False)
    ...
    for ite in range(1, valid_num + 1):
        ...
        train_iter = dataset.Iterator(train_data, src_vocab, trg_vocab, batch_size, gpu_id, sort=True)
    ...
    dataset.save_list(model_save_dir + 'label.txt', label)
    dataset.save_list(model_save_dir + 'align.txt', align)
    dataset.save_list(model_save_dir + 'tf.txt', tf)
